...
def main():
    if len(sys.argv) < 3:
        print("Usage: python devacc.py <inputfile>.txt {DevAcc,DevPr,DevRecall,DevF1,DevLoss,...} {...}")
        exit()
    ...
    def plot(s):
        ...
        if havedisp:
            ...
            fig.savefig(f"plots/{dataset}-{date}-{s}-dropouts-{dropouts[0]}-{dropouts[-1]}.png")
                     
            
    for s in sys.argv[len(dropouts)+1:]:
        disp(s)
        plot(s)
